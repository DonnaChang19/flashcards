...
(function() {
  ...
  const AuthGate = {
    ...
    injectModal() {
      ...
      wrap.innerHTML = `
        ...
        <div class="auth-card">
          ...
          <div class="auth-hint">老师后台可查看学习进度、完成率与高频错词。</div>
          <div class="auth-local" style="margin-top:12px;text-align:center;">
            <button id="authLocalBtn" class="btn ghost small">暂不登录，本地试用</button>
          </div>
        </div>
      `;
    ...
    // 进入纯本地模式（未配置 key 或用户点「本地试用」）
    enterLocalMode() {
      this.hideModal();
      if (typeof toast === 'function') toast('本地试用模式：进度仅存本机');
    ...
    async start(options) {
      ...
      if (!CloudStore.enabled || !CloudStore.sb) {
        console.info('[AuthGate] Supabase not available, entering local-only mode.');
        if (this.onSuccess) this.onSuccess({ localOnly: true });
        return;
      }
